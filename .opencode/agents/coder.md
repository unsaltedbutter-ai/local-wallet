...
- Follow the layout and invariants in PROJECT.md and AGENTS.md. The ones you are most likely to violate: network I/O only in `src/localwallet/chain/`; all model output is untrusted input (GBNF grammar → pydantic → business rules → allowlist dispatch); mainnet-only (ADR-0021: refuse testnet keys/addresses); never log xpubs, addresses, or amounts.
