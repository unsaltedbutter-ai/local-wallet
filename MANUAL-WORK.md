...
4. **PRIVACY (your report, → TCK-PRIVACY-001, high priority):** entering the zpub during onboarding appears to immediately query public mempool.space. Fix planned: zero public access until you explicitly press a "use public server" consent button; node info collected alongside the zpub in the same first-run step. If you can reproduce: note WHAT you clicked after entering the zpub (balance button? closed the pane? waited?) — that identifies which call leaked.
...
- [ ] **Private mempool (Esplora)**: in Settings → chain base, enter `https://evil-star.local:56191`. Start9's self-signed cert needs `LOCALWALLET_TLS_VERIFY=false` (env or repo-root `config.json`, TCK-CFG-003) — expect one honest warning line about unverified transport. Apply → NO restart needed: the client hot-swaps and a resync fires against your node. (ROUND-2 ROOT CAUSE 2026-09-11: your mempool returns `[]` for /api/blocks/tip and a list-wrapped genesis — TCK-BACKEND-004 fixed the shape tolerance + probe latency; re-test on the newest code.)
...
- [ ] **Round 2 — run these on the VPN machine and paste ALL output (value-free, safe):**
  1. Confirm the flag was set this run: `echo $LOCALWALLET_TLS_VERIFY` (must print 0/false for Start9 self-signed certs; if unset, set it in `config.json` AT THE REPO ROOT next to the code as `"tls_verify": false` and restart — the old `~/.localwallet/config.json` location is no longer read, TCK-CFG-003).
