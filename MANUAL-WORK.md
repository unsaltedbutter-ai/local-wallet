...
6. **Public-broadcast fallback offer coverage on electrum** (PUBLICBCAST-001). The offer fires only when the rejection is provably fee-floor-shaped. Options: (a) bitcoind-only proof (the original implementation — the offer would never fire on your electrum setup); (b) also accept the electrum server's advertised `relayfee` as the floor proof (deprecated/ambiguous field, so it's used ONLY for this consent-gated offer, never for fee bidding). **Chose (b)** — your MW-9 electrum flow gets the mempool.space fallback offer when your server advertises relayfee; if it doesn't advertise one, no offer (still fail-closed).
