...
## §INTENT-INTERPRETATION & MODEL STATUS — experiments run, results, and what's left (2026-09-16)

Your questions were: is the system prompt too large (should intent detection be split across several serial queries?), would other models do better, and could multi-prompt enable a plugin architecture. All three were investigated and **measured on your machine's class of hardware** — this section is the running record. Raw data lives in `evals/results/*.md`; the harness is `tools/probe_prompt_routing.py` (reusable: `--model <manifest-name>` runs any candidate).

**How accuracy is measured:** 106 test prompts (78 golden + 28 red-team) through the real model; scored two ways — INTENT-ONLY (did it pick the right action, e.g. "create a transaction" vs "show balance") and FULL-ENVELOPE (the enforced Phase-6 gate: right action AND right parameters). Latency = seconds per prompt on the Mac-mini-class CPU.

### What we tried — results table (intent-only accuracy, 106 prompts)

| Arm | Model | Accuracy | Sec/prompt (mean) | Verdict |
|---|---|---|---|---|
| Full system prompt (14,932 chars ≈ 3.9K tokens) | gemma-4-E2B (pinned) | **59.4%** (golden 70.5% / redteam 28.6%) | 34.9 | baseline |
| Tiny "stage-1" prompt (1,561 chars, intent names only) | gemma-4-E2B | **54.7%** | 3.8 | splitting does NOT help routing |
| Serial two-stage (classify → then params) | gemma-4-E2B | (same two numbers chained) | 38.7 (+3.8s for the extra pass) | accuracy unchanged, latency up |
| Full prompt | **Qwen3-4B-Instruct-2507** | **68.9%** (golden 79.5% / redteam 39.3%) | 93.6 | best accuracy, **2.7× too slow** |
| Full prompt | Qwen3-1.7B | 59.4% (identical to E2B) | 43.6 | no gain, slower |
| Full prompt | Phi-4-mini | 60.4% | 49.8 | no meaningful gain, slower |
| Embedding similarity router (Qwen3-Embedding-0.6B, 66 reference phrases, cosine match) | — | **47.2%** | 0.16 | fast but worse; hybrid gains nothing (zero low-confidence cases to fall back on) |

Decision rule for swapping models was fixed in advance: ≥ +15 accuracy points AND ≤ 45s serial-two-stage. **Nothing cleared it → the pinned gemma-4-E2B stays.** Your 8 suggested models were all embedding models (they output vectors, not text — they cannot generate the transaction envelopes); only Qwen3-Embedding-0.6B was runnable (the NVIDIA ones need Blackwell-GPU formats or lack GGUF builds; the Jina ones are non-commercial-licensed or gated) and it lost to the LLM router.

**Key finding:** the binding constraint is the ~2B model's comprehension, NOT prompt size or structure. The same model misses the same prompts regardless of prompt size (T1) and regardless of sampling temperature (earlier TCK-AGT-001 finding). The accepted full-envelope record on the pinned model remains 26/53 = 49.1% (older 53-prompt corpus) — failures are spread across all intents, not concentrated where a prompt split would help.

**Plugin architecture verdict (design study):** multi-prompt gives ZERO plugin surface — prompt text can't add capabilities; the security boundary is the dispatcher (closed intent enum + validators), not the prompt. A safe 3rd-party plugin API would have to be built on the dispatcher (out-of-process, read/compute-only handlers) and is judged NOT justified for a single-user local tool today; the honest extension path is curated first-party "intent packs" through the existing review process — which is how the registry already grew 3 → 15 intents. Also: with the model routing at ~59%, piling MORE model-routed intents on makes things worse — the app's real pattern is deterministic intercepts (11 of them) that answer before the model ever sees the text.

### Not yet tried (in rough value order)

1. **Qwen3-4B full-envelope gate run** — the bake-off only measured intent routing; the 4B's +9.5 points might translate into a materially better full gate IF you accept ~90s turns (or run it on faster hardware). Cheapest next experiment; the model is already downloaded.
2. **Quantization test**: gemma-4-E2B at Q8_0 instead of Q4_0 (~2× size, maybe +1-3 points, modest slowdown) — untested.
3. **Single-pass prompt de-bloat** (move per-intent phrasings into examples) — the no-new-machinery option from the design study; low expected gain per the T1 evidence.
4. **Fine-tuning / LoRA on our own fixtures** — probably the real accuracy unlock for a small model, but it's a training pipeline, a bigger commitment, and unproven for grammar-constrained decoding.
